//------------------------------------------------------------------------------
// ClientLogic.cs
//
// <copyright from='2017' to='2117' company='SF Technology'> 
// Copyright (c) SF Technology. All Rights Reserved. 
// Information Contained Herein is Proprietary and Confidential. 
// </copyright>
//
// Created: 01/30/2017
// Owner: HongYin Wang
//
//------------------------------------------------------------------------------

using System;
using System.Collections.Generic;
using System.Linq;

using SF.ILogic;
using SF.IService;
using SF.Service.UnitOfWork;
using SF.Domain;
using SF.Domain.ResultModels;
using SF.Logic.Common;
using SF.Logic.ModelConverter;
using SF.Repositoriy.Entities;

namespace SF.Logic
{
    public class ClientLogic : IClientLogic
    {
        private readonly IClientService _clientService;
        private readonly IUnitOfWorkFactory _unitOfWorkFactory;

        public ClientLogic(IClientService clientService, IUnitOfWorkFactory unitOfWorkFactory)
        {
            _clientService = clientService;
            _unitOfWorkFactory = unitOfWorkFactory;
        }

        public void Create(ClientDomainModel model)
        {
            using (var unitWork = _unitOfWorkFactory.GetCurrentUnitOfWork())
            {
                _clientService.Create(model.ToDataModel());
                unitWork.Commit();
            }
        }

        public void Edit(ClientDomainModel model)
        {
            using (var unitWork = _unitOfWorkFactory.GetCurrentUnitOfWork())
            {
                _clientService.Edit(model.ToDataModel());
                unitWork.Commit();
            }
        }

        public void Delete(int id)
        {
            using (var unitWork = _unitOfWorkFactory.GetCurrentUnitOfWork())
            {
                _clientService.Delete(id);
                unitWork.Commit();
            }
        }

        public ClientDomainModel Get(int id)
        {
            var user = _clientService.Get(id);
            return user.ToDomainModel();
        }

        public List<ClientDomainModel> GetAll()
        {
            var model = _clientService.Query();
            return !model.Any()
                ? null
                : model.ToList()
                    .Select(m => m.ToDomainModel())
                    .ToList();
        }
        
        public PagedList<ClientDomainModel> GetClients(ClientQueryModel queryModel)
        {
            var source = _clientService.Query();

            if (!string.IsNullOrEmpty(queryModel.Name))
            {
                source = source.Where(x => x.Name.Contains(queryModel.Name));
            }

            if (queryModel.ClientId > 0)
            {
                source = source.Where(x => x.ClientId == queryModel.ClientId);
            }

            if (queryModel.IsEnabled.HasValue)
            {
                source = source.Where(x => x.IsEnabled == queryModel.IsEnabled);
            }

            if (string.IsNullOrEmpty(queryModel.OrderBy))
            {
                queryModel.OrderBy = "ClientId";
            }

            var pageCount = source.Count();

            source = queryModel.IsDesc
                    ? source.SortByDescending(queryModel.OrderBy)
                    : source.SortBy(queryModel.OrderBy);

            var pageItems = source.Skip(queryModel.PageIndex * queryModel.PageSize)
                .Take(queryModel.PageSize)
                .ToList()
                .Select(x => x.ToDomainModel())
                .ToList();
            var result = new PagedList<ClientDomainModel>(pageItems, queryModel.PageIndex,
                queryModel.PageSize, pageCount);

            return result;
        }
    }
}
